...
export default function Menu() {
  ...
  return (
    <div className="bg-darkGrey w-[264px] relative rounded-lg shadow-menu py-4 px-6">
      <FontAwesomeIcon
        icon={faXmark}
        onClick={() => setDisplayMenuModal(false)}
        className="text-mediumGrey w-4 h-4 absolute right-0 mr-4 cursor-pointer"
      />
      <h3 className="text-mediumGrey text-xs font-semibold tracking-[2.4px] uppercase mb-4">
        ...
        {boards?.map((board, ind) => (
          <div className="flex items-center gap-3 py-3">
            ...
            <Link
              ...
              className="text-mediumGrey text-[15px] font-semibold"
            >
              {board.name}
            </Link>
          </div>
      ...
      <div className="text-mediumGrey flex items items-center justify-end gap-2 mt-4">
        <FontAwesomeIcon icon={faArrowRightFromBracket} />
        <Link to="/logout" className="text-[13px] font-semibold">
          Logout
        </Link>
      </div>
    </div>
